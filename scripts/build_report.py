...
def build_anual_averias_section(cache, tiempos_rows=None):
    ...
    html = (
        ...
        '  var tfT = tf.querySelector(".tickets-cell");'
        '  tfT.textContent = totals._t;'
        '  tfT.style.cursor = totals._t ? "pointer" : "default";'
        '  tf.querySelectorAll(".bucket-cell").forEach(function(td){'
        '    var v = totals[td.getAttribute("data-bucket")] || 0;'
        '    td.textContent = v || "·";'
        '    td.style.cursor = v ? "pointer" : "default";'
        '  });'
        '}'
        ...
        '  if(!tr) return;'
        '  if(typeof window.avd_setExtraFilter !== "function") return;'
        '  var isTotal = tr.parentElement && tr.parentElement.tagName === "TFOOT";'
        '  var y = tr.getAttribute("data-year");'
        '  if(!isTotal && !y) return;'
        '  if(isTotal) {'
        '    /* Total tickets: limpia filtro (muestra todos) */'
        '    if(td.classList.contains("tickets-cell")) {'
        '      cellFilter = null;'
        '      if(window.avd_clearExtraFilter) window.avd_clearExtraFilter();'
        '      var tt=document.getElementById("avd-table"); if(tt) tt.scrollIntoView({behavior:"smooth",block:"start"});'
        '      return;'
        '    }'
        '    /* Total bucket: filtra solo por bucket (todos los años) */'
        '    if(td.classList.contains("bucket-cell")) {'
        '      var b = td.getAttribute("data-bucket");'
        '      cellFilter = {y: null, b: b};'
        '      window.avd_setExtraFilter(function(row){'
        '        var buckets = (row.getAttribute("data-av-buckets") || "").split(",");'
        '        return buckets.indexOf(b) >= 0;'
        '      }, "bucket=" + b + " (todos los años)");'
        '      return;'
        '    }'
        '    return;'
        '  }'
...
def build_tiempos_section(cache, sustis_by_parent):
    ...
    # EXCLUIR los gestionados Online y Externa (la máquina no se gestiona en taller físico).
    # Solo cuentan Interna (Reparación en taller) e Inicio (sin asignar todavía).
    _excluded_gestion = {"Online", "Externa"}
...
def build_tecnicos_mensual_section(cache, tiempos_rows=None):
    ...
    # Per-ticket events: lista de (mes, tipo, fecha_iso)
    # tec se determina como tec_taller actual (heurística aceptada)
    ticket_events = {}  # k -> list of (month, "P"/"R", "YYYY-MM-DD")
    ticket_tec = {}     # k -> tec actual
    active_now = {}     # tec -> count tickets activos en taller hoy
    tecs_set = set()
    months_set = set()
    dates_set = set()   # set de "YYYY-MM-DD" con al menos un evento
    ...
    for k, t in cache.get("tickets", {}).items():
        ...
        for tr in t.get("transitions", []):
            ...
            if to == PRES_STATE and first_pres_dt is None:
                first_pres_dt = dt
            elif to == REP_STATE and first_rep_dt is None:
                first_rep_dt = dt
        evs = []
        if first_pres_dt:
            iso = first_pres_dt.date().isoformat()
            evs.append((first_pres_dt.month, "P", iso))
            months_set.add(first_pres_dt.month); tecs_set.add(tec); dates_set.add(iso)
        if first_rep_dt:
            iso = first_rep_dt.date().isoformat()
            evs.append((first_rep_dt.month, "R", iso))
            months_set.add(first_rep_dt.month); tecs_set.add(tec); dates_set.add(iso)
        if evs:
            ticket_events[k] = evs
    ...
    for m in months_sorted:
        ...
        for tec in tecs_sorted:
            ...
            # Total número encima — clickable para filtrar Total P+R sin separar kind
            if total > 0:
                svg.append(
                    f'<text class="tec-bar-total" data-tec="{tec_esc}" data-mes="{m}" '
                    f'x="{x + BAR_W/2:.1f}" y="{_y(total) - 4:.1f}" text-anchor="middle" '
                    f'fill="#334155" font-weight="700" font-size="11" style="cursor:pointer;text-decoration:underline">'
                    f'<title>{tec_esc} · {MONTH_LABELS[m-1]} · Total P+R: {total} (clic = sin separar P/R)</title>'
                    f'{total}</text>'
                )
    ...
    if tiempos_rows:
        ...
        def _extra_tec(r):
            ...
            meses_csv = ",".join(str(m) for (m, _, _) in evs)
            kinds_csv = ",".join(kind for (_, kind, _) in evs)
            dates_csv = ",".join(iso for (_, _, iso) in evs)
            return {
        ...
        # Filtro Día: dropdown con todas las fechas con eventos (desc), para filtrar el detail
        dates_sorted = sorted(dates_set, reverse=True)
        day_opts = ''.join(f'<option value="{d}">{d}</option>' for d in dates_sorted)
        day_filter_html = (
            '<div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;margin:10px 0 4px;padding:9px 12px;background:#f1f5f9;border:1px solid var(--line);border-radius:7px;font-size:12px">'
            '<label style="display:inline-flex;align-items:center;gap:6px"><span style="color:#475569">Filtrar por día:</span>'
            '<select id="tec-day-filter" style="padding:4px 8px;border:1px solid var(--line);border-radius:4px;font-size:12px;background:white">'
            '<option value="">Todos los días</option>'
            + day_opts +
            '</select></label>'
            '<button type="button" id="tec-day-clear" style="border:1px solid var(--line);background:white;border-radius:4px;padding:4px 10px;font-size:12px;cursor:pointer">✕ Quitar día</button>'
            '<span style="margin-left:auto;color:#475569">Clic en segmento P/R = filtra por tec+mes+kind · Clic en el <b>número de arriba</b> = Total P+R sin separar</span>'
            '</div>'
        )
        intro = (
    ...
    chart_js = (
        ...
        'ev.stopPropagation();'
        'var tec=el.getAttribute("data-tec");var mes=el.getAttribute("data-mes");'
        'if(typeof window.tec_setExtraFilter!=="function") return;'
        'var lbl="téc="+tec+" · mes="+monthLabels[parseInt(mes,10)-1]+" · Total P+R";'
        'window.tec_setExtraFilter(function(r){'
        'var rtec=r.getAttribute("data-tec-tec")||"";'
        'var rmeses=(r.getAttribute("data-tec-meses")||"").split(",");'
        'if(rtec!==tec)return false;'
        ...
        'if(!perTec[e.tec])perTec[e.tec]={p:0,r:0};'
        'if(e.kind==="P")perTec[e.tec].p++;else perTec[e.tec].r++;'
        '});'
        'var tecs=Object.keys(perTec).sort();'
        '/* SVG bar chart estilo monthly pero con UN solo grupo (el día) */'
        'var BAR_W=24,BAR_GAP=4,LEFT_PAD=56,RIGHT_PAD=20,PLOT_H=320,X_LABEL_H=80,TOP_PAD=20;'
        'var maxTot=1;tecs.forEach(function(t){var tot=perTec[t].p+perTec[t].r;if(tot>maxTot)maxTot=tot;});'
        'var groupW=tecs.length*BAR_W+Math.max(0,tecs.length-1)*BAR_GAP;'
        'var svgW=Math.max(LEFT_PAD+groupW+RIGHT_PAD,400);'
        'var svgH=TOP_PAD+PLOT_H+X_LABEL_H;'
        'function _y(v){return TOP_PAD+PLOT_H-(v/maxTot)*PLOT_H;}'
        'var PRES="#0891b2",REP="#059669";'
        'var svg=[\'<svg viewBox="0 0 \'+svgW+\' \'+svgH+\'" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:\'+svgW+\'px;display:block;font-family:Inter,sans-serif;font-size:11px">\'];'
        '/* Eje Y */ '
        'for(var i=0;i<=5;i++){var yv=(maxTot/5)*i;var py=_y(yv);'
        'svg.push(\'<line x1="\'+LEFT_PAD+\'" y1="\'+py+\'" x2="\'+(svgW-RIGHT_PAD)+\'" y2="\'+py+\'" stroke="#e5e7eb" stroke-width="1"/>\');'
        'svg.push(\'<text x="\'+(LEFT_PAD-6)+\'" y="\'+(py+4)+\'" text-anchor="end" fill="#64748b">\'+Math.round(yv)+\'</text>\');}'
        '/* Barras */ '
        'var x=LEFT_PAD;'
        'tecs.forEach(function(t){'
        'var p=perTec[t].p,r=perTec[t].r,tot=p+r;'
        'if(r>0){var yR=_y(tot),hR=_y(p)-_y(tot);'
        'svg.push(\'<rect class="tec-bar" data-tec="\'+t+\'" data-mes="" data-kind="R" data-date="\'+d+\'" x="\'+x+\'" y="\'+yR+\'" width="\'+BAR_W+\'" height="\'+hR+\'" fill="\'+REP+\'" style="cursor:pointer"><title>\'+t+\' · \'+d+\' · Reparados: \'+r+\'</title></rect>\');}'
        'if(p>0){var yP=_y(p),hP=_y(0)-_y(p);'
        'svg.push(\'<rect class="tec-bar" data-tec="\'+t+\'" data-mes="" data-kind="P" data-date="\'+d+\'" x="\'+x+\'" y="\'+yP+\'" width="\'+BAR_W+\'" height="\'+hP+\'" fill="\'+PRES+\'" style="cursor:pointer"><title>\'+t+\' · \'+d+\' · Pptos: \'+p+\'</title></rect>\');}'
        'if(tot>0){svg.push(\'<text class="tec-bar-total" data-tec="\'+t+\'" data-mes="" data-date="\'+d+\'" x="\'+(x+BAR_W/2)+\'" y="\'+(_y(tot)-4)+\'" text-anchor="middle" fill="#334155" font-weight="700" font-size="11" style="cursor:pointer;text-decoration:underline"><title>\'+t+\' · \'+d+\' · Total P+R: \'+tot+\'</title>\'+tot+\'</text>\');}'
        '/* Label tec rotada */ '
        'var tx=x+BAR_W/2;var ty=TOP_PAD+PLOT_H+8;'
        'svg.push(\'<text x="\'+tx+\'" y="\'+ty+\'" text-anchor="end" fill="#475569" transform="rotate(-55 \'+tx+\' \'+ty+\')" font-size="10">\'+t.substring(0,16)+\'</text>\');'
        'x+=BAR_W+BAR_GAP;'
        '});'
        '/* Etiqueta día grande */ '
        'svg.push(\'<text x="\'+(svgW/2)+\'" y="\'+(svgH-6)+\'" text-anchor="middle" fill="#0b3d91" font-weight="700" font-size="13">Día \'+d+\'</text>\');'
        'svg.push(\'</svg>\');'
        'var header=\'<div style="margin:6px 0 10px;padding:8px 12px;background:#dbeafe;border:1px solid #93c5fd;border-radius:6px;font-size:13px"><b style="color:#0b3d91">Vista del día \'+d+\'</b> &middot; \'+filtered.length+\' eventos · clic en barras filtra el detail</div>\';'
        'if(tecs.length===0){dayEl.innerHTML=header+\'<div style="padding:20px;color:#94a3b8;font-style:italic;text-align:center">Sin eventos ese día</div>\';}'
        'else{dayEl.innerHTML=header+svg.join("");}'
        'mainEl.style.display="none";'
        'dayEl.style.display="";'
        '/* Rebind clicks en las nuevas barras (mismo comportamiento que las del chart mensual) */ '
        'dayEl.querySelectorAll(".tec-bar").forEach(function(el){'
        'el.addEventListener("click",function(){'
        'var tec=el.getAttribute("data-tec");var kind=el.getAttribute("data-kind");'
        'if(typeof window.tec_setExtraFilter!=="function") return;'
        'window.tec_setExtraFilter(function(r){'
        'var rtec=r.getAttribute("data-tec-tec")||"";'
        ...
        'var rkinds=(r.getAttribute("data-tec-kinds")||"").split(",");'
        'if(rtec!==tec)return false;'
        'if(rdates.indexOf(d)<0)return false;'
        'if(rkinds.indexOf(kind)<0)return false;'
        'return true;'
        '},"téc="+tec+" · día="+d+" · "+(kind==="P"?"Pptos":"Reparaciones"));'
        '});});'
        'dayEl.querySelectorAll(".tec-bar-total").forEach(function(el){'
        'el.addEventListener("click",function(ev){ev.stopPropagation();'
        'var tec=el.getAttribute("data-tec");'
        'if(typeof window.tec_setExtraFilter!=="function") return;'
        'window.tec_setExtraFilter(function(r){'
        'var rtec=r.getAttribute("data-tec-tec")||"";'
        'var rdates=(r.getAttribute("data-tec-dates")||"").split(",");'
        'return rtec===tec && rdates.indexOf(d)>=0;'
        '},"téc="+tec+" · día="+d+" · Total P+R");'
        '});});'
        '}'
